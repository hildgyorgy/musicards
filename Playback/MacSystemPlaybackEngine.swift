...
final class MacSystemPlaybackEngine: PlaybackEngine {
    ...
    func prepare(_ item: PlaybackQueueItem) async throws {
        guard case .localFile(let url) = item.source else {
            throw MacPlaybackEngineError("Unsupported audio source")
        }
        ...
        let decodedPCM = try await Task.detached(priority: .userInitiated) {
            try MacAudioFileDecoder.decode(url: url)
        }.value
    ...
    func play() async throws {
        guard let decodedPCM, let outputUnit else {
            throw MacPlaybackEngineError("No decoded track is ready")
        }
    ...
    private func check(_ status: OSStatus, operation: String) throws {
        guard status == noErr else {
            throw MacPlaybackEngineError(operation, status: status)
        }
    }
}
#endif
